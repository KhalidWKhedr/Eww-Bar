 ...
 (box :class "container_devices"
   ...
   (box :class "container_options"
     ...
     (button :class "bluetooth_link"
             ...
             :onclick "${HOME}/.config/eww/scripts/bluetooth/device_controller/main.sh  pair \"$mac_address\""
             "")
     (button :class "bluetooth_connect"
             :style "font-size: 20px; padding-left: 0px"
             :onclick "${HOME}/.config/eww/scripts/bluetooth/device_controller/main.sh  connect \"$mac_address\""
             "󰂱")
     (button :class "bluetooth_trust"
             :style "font-size: 15px; padding-left: 0px"
             :onclick "${HOME}/.config/eww/scripts/bluetooth/device_controller/main.sh trust \"$mac_address\""
             "")))
INNER
)"
    done <<< "$devices"
